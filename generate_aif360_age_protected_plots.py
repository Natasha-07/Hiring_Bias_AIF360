"""Generate before/after AIF360 results with Age as the protected attribute.

Protected attribute:
- privileged group: Age == "<35"
- unprivileged group: Age != "<35" (typically ">35")
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate age-protected AIF360 before/after plots.")
...
def save_model_comparison_plot(before_df: pd.DataFrame, after_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    fig.suptitle("Age-Protected AIF360: Model Comparison", y=1.08, fontsize=14)
    fig.tight_layout()
    fig.savefig(out_dir / "aif360_age_protected_model_metrics_before_after.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def save_age_group_plot(pred_df: pd.DataFrame, out_dir: Path, model_name: str) -> None:
    grouped = pred_df.groupby("Age")[["Pred_Before", "Pred_After"]].mean()
    age_order = [age for age in ["<35", ">35", "Unknown"] if age in grouped.index]
    ...
    ax.set_title(f"Predicted Hiring Rate by Age ({model_name})")
    ax.grid(axis="y", alpha=0.25)
    ax.legend()
    plt.tight_layout()
    fig.savefig(out_dir / "aif360_age_protected_pred_hiring_rate_by_age_before_after.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    age_raw = original_df["Age"].fillna("Unknown").astype(str).str.strip()

    # privileged = <35, unprivileged = >35/other
    analysis_df = analysis_df.copy()
    analysis_df["Is_Under35"] = (age_raw == "<35").astype(float)
    ...
    bld = BinaryLabelDataset(
        ...
        protected_attribute_names=["Is_Under35"],
    )
    privileged_groups = [{"Is_Under35": 1}]
    unprivileged_groups = [{"Is_Under35": 0}]
    ...
    pred_df = pd.DataFrame(
        {
            "Age": age_raw.iloc[test_indices].values,
    ...
    save_age_group_plot(pred_df, args.out_dir, best_model)

    before_df.to_csv(args.out_dir / "aif360_age_protected_model_metrics_before.csv")
    after_df.to_csv(args.out_dir / "aif360_age_protected_model_metrics_after.csv")
    combined = before_df.add_suffix("_before").join(after_df.add_suffix("_after"))
    combined.to_csv(args.out_dir / "aif360_age_protected_all_models_before_after.csv")

    pred_df.groupby("Age")[["Pred_Before", "Pred_After"]].mean().to_csv(
        args.out_dir / "aif360_age_protected_pred_rate_by_age_before_after.csv"
    )
    pred_df.groupby("Age").size().rename("n").to_csv(
        args.out_dir / "aif360_age_protected_test_counts_by_age.csv"
    )

    print(f"Saved age-protected AIF360 outputs to: {args.out_dir}")
    print(f"Best baseline model for age group plot: {best_model}")
