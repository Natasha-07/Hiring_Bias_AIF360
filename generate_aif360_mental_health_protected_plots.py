"""Generate before/after AIF360 results with MentalHealth as the protected attribute.

Protected attribute:
- privileged group: MentalHealth == "No"
- unprivileged group: MentalHealth != "No" (typically "Yes")
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate MentalHealth-protected AIF360 before/after plots.")
...
def save_model_comparison_plot(before_df: pd.DataFrame, after_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    fig.suptitle("MentalHealth-Protected AIF360: Model Comparison", y=1.08, fontsize=14)
    fig.tight_layout()
    fig.savefig(
        out_dir / "aif360_mental_health_protected_model_metrics_before_after.png",
        dpi=180,
        bbox_inches="tight",
    )
    plt.close(fig)
...
def save_group_plot(pred_df: pd.DataFrame, out_dir: Path, model_name: str) -> None:
    grouped = pred_df.groupby("MentalHealth")[["Pred_Before", "Pred_After"]].mean()
    ...
    ax.set_title(f"Predicted Hiring Rate by MentalHealth ({model_name})")
    ax.grid(axis="y", alpha=0.25)
    ax.legend()
    plt.tight_layout()
    fig.savefig(
        out_dir / "aif360_mental_health_protected_pred_hiring_rate_by_mental_health_before_after.png",
        dpi=180,
    )
    plt.close(fig)
...
def main() -> None:
    ...
    mental_raw = original_df["MentalHealth"].fillna("Unknown").astype(str).str.strip()

    # privileged = No, unprivileged = Yes/other
    analysis_df = analysis_df.copy()
    analysis_df["Is_MH_No"] = (mental_raw.str.lower() == "no").astype(float)
    ...
    bld = BinaryLabelDataset(
        ...
        protected_attribute_names=["Is_MH_No"],
    )
    privileged_groups = [{"Is_MH_No": 1}]
    unprivileged_groups = [{"Is_MH_No": 0}]
    ...
    pred_df = pd.DataFrame(
        {
            "MentalHealth": mental_raw.iloc[test_indices].values,
    ...
    before_df.to_csv(args.out_dir / "aif360_mental_health_protected_model_metrics_before.csv")
    after_df.to_csv(args.out_dir / "aif360_mental_health_protected_model_metrics_after.csv")
    combined = before_df.add_suffix("_before").join(after_df.add_suffix("_after"))
    combined.to_csv(args.out_dir / "aif360_mental_health_protected_all_models_before_after.csv")

    pred_df.groupby("MentalHealth")[["Pred_Before", "Pred_After"]].mean().to_csv(
        args.out_dir / "aif360_mental_health_protected_pred_rate_by_mental_health_before_after.csv"
    )
    pred_df.groupby("MentalHealth").size().rename("n").to_csv(
        args.out_dir / "aif360_mental_health_protected_test_counts_by_mental_health.csv"
    )

    print(f"Saved mental-health-protected AIF360 outputs to: {args.out_dir}")
    print(f"Best baseline model for mental health plot: {best_model}")
